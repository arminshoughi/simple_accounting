...
console.log( diagram?.expenses_data
  ...
  useEffect(() => {
    ...
    diagram?.income_data?.filter(i => i?.year ===Number(year)).filter(i =>i?.month ===Number(mounth))?.forEach(entry => {
    ...
    diagram?.expenses_data?.filter(i => i?.year ===Number(year)).filter(i =>i?.month ===Number(mounth))?.forEach(entry => {
      const index = entry.day - 1; // days are 1-indexed, so subtract 1 to get the 0-indexed position
